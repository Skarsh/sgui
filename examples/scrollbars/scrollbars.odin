...
build_ui :: proc(ctx: ^ui.Context, data: ^Data) {
	...
	horizontal_boxes_wrapper_comm := ui.begin_container(
		ctx,
		ui.Style {
			sizing_x = ui.sizing_grow(),
			sizing_y = ui.sizing_grow(),
	...
	ui.container(
		...
		name = "horizontal_box_3",
	)

	horizontal_boxes_wrapper_element := horizontal_boxes_wrapper_comm.element
	ui.scrollbar(ctx, horizontal_boxes_wrapper_element, .X, name = "horizontal_scrollbar")

	ui.end_container(ctx)

	scroll_region := horizontal_boxes_wrapper_comm.element.scroll_region
	scroll_region_info_str := make_info_str(
		data.horizontal_scroll_region_str_buf,
		horizontal_boxes_wrapper_element.name,
		horizontal_boxes_wrapper_element.size,
		scroll_region,
	)
	ui.text(ctx, scroll_region_info_str, name = "scroll_box_1_scroll_region_info")
	...
	vertical_boxes_wrapper_comm := ui.begin_container(
		ctx,
		ui.Style {
			sizing_x = ui.sizing_grow(),
			sizing_y = ui.sizing_grow(),
	...
	vertical_boxes_scroll_region_info_str := make_info_str(
		...
		vertical_boxes_wrapper_element.name,
		vertical_boxes_wrapper_element.size,
		vertical_boxes_scroll_region,
	)

	ui.text(ctx, vertical_boxes_scroll_region_info_str, name = "scroll_box_2_scroll_region_info")

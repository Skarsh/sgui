...
build_ui :: proc(ctx: ^ui.Context, data: ^Data) {
	...
	green_comm := make_slider_row(ctx, "green", "G", &data.g, GREEN_COLOR, data.value_bufs[1][:])
	blue_comm := make_slider_row(ctx, "blue", "B", &data.b, BLUE_COLOR, data.value_bufs[2][:])
	alpha_comm := make_slider_row(ctx, "alpha", "A", &data.a, ALPHA_COLOR, data.value_bufs[3][:])

...
check_next_word :: proc(
	...
	for backend in TEST_BACKENDS {
		tb := test_text_buffer(backend, text)
		actual := text_buffer_next_word_byte_pos(tb, pos)
		testing.expectf(
			t,
			actual == expected,
			"[%v] next word from %v in %q: expected %v, got %v",
			backend,
			pos,
			text,
			expected,
...
check_prev_word :: proc(
	...
	for backend in TEST_BACKENDS {
		tb := test_text_buffer(backend, text)
		actual := text_buffer_prev_word_byte_pos(tb, pos)
		testing.expectf(
			t,
			actual == expected,
			"[%v] prev word from %v in %q: expected %v, got %v",
			backend,
			pos,
			text,
			expected,
